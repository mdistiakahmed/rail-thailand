...
export const SearchStationButton: React.FC<SearchStationButtonProps> = ({ stations, routes }) => {
  ...
  const handleSearch = () => {
    if (startStation && !destinationStation) {
      ...
      const slug = formatStationNameForUrl(startStation);
      router.push(`/stations/${slug}`);
    } else if (startStation && destinationStation) {
      // Both stations - find matching route
      const routeName = `${startStation} - ${destinationStation}`;
      const matchingRoute = routes.find(route => route === routeName);
      
      if (matchingRoute) {
        const filename = createFilenameFromRoute(matchingRoute);
        const startStationSlug = formatStationNameForUrl(startStation);
        router.push(`/stations/${startStationSlug}/${filename}`);
      } else {
