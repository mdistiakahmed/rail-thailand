...
function generateScheduleParagraph(trips: any[], from: string, to: string) {
  ...
  return (
    <p className="my-6 text-gray-700 leading-relaxed text-center max-w-4xl mx-auto">
      The {from} to {to} train schedule includes {trips.length} daily services
      operated by the State Railway of Thailand (SRT). This timetable covers
      express, rapid, and ordinary trains connecting {from} station and {to}{" "}
      station throughout the day. For example, {examples}, providing passengers
      with flexible travel options and reliable arrival times across the
...
export default async function CityRoutePage({ params }: any) {
  ...
  const structuredData = generateRouteStructuredData(fromCity, toCity, forward);
    ...
    return (
      <div className="text-center mb-10 max-w-4xl mx-auto px-4">
        ...
        <p className="text-lg mb-4 text-gray-700">
          The{" "}
          <strong>
            {fromCity} to {toCity} train schedule
          </strong>{" "}
          includes {forward.length} daily train services with departure times
          starting from {firstDeparture} and running until {lastDeparture}. This
        ...
        <p className="text-base mb-4 text-gray-600">
          Whether you are checking the latest {fromCity} to {toCity} train time
          or comparing express, rapid, and ordinary SRT trains, this Thailand
          Railway schedule helps you plan your trip efficiently between{" "}
          {fromCity} station and {toCity} station.
        </p>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen w-screen md:w-full py-8 md:px-4 ">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 overflow-x-auto">
        ...
        {forward.length > 0 && generateRouteFAQ(forward, fromCity, toCity)}
